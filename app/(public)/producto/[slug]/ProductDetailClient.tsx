...
export function ProductDetailClient({
  ...
  const [variantsError] = useState(initialVariantsError)
  const [related] = useState<Product[]>(initialRelated)
  const loading = false
  const cartItems = useCartStore((s) => s.items)

  const [selectedVariant, setSelectedVariant] = useState<ProductVariant | null>(null)
  const [primaryIndex, setPrimaryIndex] = useState(initialProduct.primary_image_index ?? 0)
    ...
    <div className={cn('flex items-center gap-2.5', className)}>
      ...
      <motion.button
        ...
        className={cn(
          ...
          (!canAddToCart || selectedVariant?.stock === 0) ? 'bg-gray-200 text-gray-400 cursor-not-allowed' :
            ...
            <div>
              ...
              <div className="relative mt-1">
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(product.description ?? '', { ALLOWED_TAGS: ['p','br','b','i','strong','em','ul','ol','li','h2','h3','a'], ALLOWED_ATTR: ['href','target','rel'] }) }}
                />
      ...
      <div className="fixed bottom-0 inset-x-0 z-40 sm:hidden bg-white border-t border-gray-100 shadow-xl">
        <div className="flex items-center gap-2 px-3 py-2.5 pb-[calc(0.625rem+env(safe-area-inset-bottom))]">
          ...
          >
            ...
            <span>{added ? 'Listo' : selectedVariant?.stock === 0 ? 'Agotada' : needsVariantSelection ? 'Elegir' : 'Agregar'}</span>
          </motion.button>
        </div>
      </div>
