...
import { getCustomerCohorts } from '@/lib/supabase/queries/analytics'
...
export async function GET(request: NextRequest) {
  ...
  const parsed = schema.safeParse({
    dateFrom: request.nextUrl.searchParams.get('dateFrom') ?? new Date(Date.now() - 365 * 86400000).toISOString().slice(0, 10),
  ...
  const data = await getCustomerCohorts(supabase, parsed.data)
