    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className={cn(
            'hidden sm:flex h-8 px-3 rounded-full text-xs font-bold border transition-colors',
            profile.is_active
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input type="number" value={couponPct} onChange={(e) => setCouponPct(e.target.value)} className="h-9 text-sm" min={0} max={100} />
              </div>
              <div>
                <label className="text-[10px] font-semibold text-gray-400 block mb-1">Por link (%)</label>
                <Input type="number" value={cookiePct} onChange={(e) => setCookiePct(e.target.value)} className="h-9 text-sm" min={0} max={100} />
              </div>
            </div>
            <Button onClick={handleSaveRates} disabled={saving} size="sm" className="mt-3 w-full h-8 rounded-xl text-xs font-semibold">
