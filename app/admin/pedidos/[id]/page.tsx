...
import {
  ArrowLeft, ChevronLeft, ChevronRight, ChevronDown, Package,
  Phone, Mail, MapPin, Copy, Send, Printer, Download, Loader2,
  Clock, CreditCard, Truck, CheckCircle2, XCircle, AlertTriangle,
  RotateCcw, Ban, MessageSquare, FileText, ArrowRight, X,
} from 'lucide-react'
...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const changeStatus = async () => {
    if (!order || !confirmNewStatus) return
    ...
    try {
      ...
      toast.success(`Estatus actualizado: ${sMeta(confirmNewStatus as OrderStatus).label}`)
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <a href={`/admin/pedidos/print?ids=${order.id}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 h-9 rounded-xl border border-gray-200 bg-white px-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition">
            <Printer className="h-4 w-4" /> Imprimir surtido
          </a>

          {transitions.length > 0 && (
            <DropdownMenu>
              <DropdownMenuTrigger>
                <span className="inline-flex items-center gap-1.5 h-9 rounded-xl bg-primary-dark px-4 text-sm font-semibold text-white hover:bg-primary-dark/90 transition cursor-pointer">
                  Cambiar estatus <ChevronDown className="h-3.5 w-3.5" />
                </span>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56">
                {transitions.map((ns) => (
                  <DropdownMenuItem key={ns} onClick={() => { setConfirmNewStatus(ns); setConfirmNote(''); setConfirmOpen(true) }} className="gap-2 text-sm cursor-pointer">
                    {sIcon(ns)} {sMeta(ns).label}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            </DropdownMenu>
          ...
          {canCancel && (
            <Button variant="outline" onClick={() => { setCancelReason(''); setCancelOpen(true) }} className="h-9 rounded-xl text-sm gap-1.5 text-red-600 border-red-200 hover:bg-red-50 hover:text-red-700">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <p className="text-sm font-semibold text-gray-900">Productos ({items.length})</p>
            </div>
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50/80 hover:bg-gray-50/80">
                  ...
                  <TableHead className="text-xs font-semibold uppercase tracking-wide text-gray-500 text-right">Precio unit.</TableHead>
            ...
            <div className="border-t border-gray-100 px-5 py-4 space-y-2">
              ...
              {order.coupon_discount > 0 && <div className="flex justify-between text-sm"><span className="text-gray-500">Cupón ({order.coupon_code})</span><span className="tabular-nums text-red-600">-{formatPrice(order.coupon_discount)}</span></div>}
                  ...
                  {updates.map((u) => {
                    ...
                    return (
                      <div key={u.id} className="relative">
                        <div className={cn('absolute -left-7 top-0.5 flex h-5 w-5 items-center justify-center rounded-full', isNote ? 'bg-amber-100 text-amber-600' : 'bg-gray-100 text-gray-500')}>
                          {isNote ? <MessageSquare className="h-2.5 w-2.5" /> : sIcon(u.status as OrderStatus)}
                        </div>
                        <div>
                          <p className="text-sm text-gray-800">{u.message}</p>
                          <p className="text-[11px] text-gray-400 mt-0.5">{formatDate(u.created_at)} · {u.updated_by ?? 'sistema'}</p>
                        </div>
                      </div>
                    )
                  })}
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-5 space-y-3">
            ...
            {order.customer_email && (
              <a href={`mailto:${order.customer_email}`} className="flex items-center gap-1.5 text-xs text-gray-600 hover:text-primary-cyan transition">
                <Mail className="h-3 w-3" /> {order.customer_email}
              </a>
            )}
            {order.customer_phone && (
              <a href={`tel:${order.customer_phone}`} className="flex items-center gap-1.5 text-xs text-gray-600 hover:text-primary-cyan transition">
          ...
          <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-5 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={copyAddress} className="p-1 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition" title="Copiar">
          ...
          {/* Payment */}
          <div className="rounded-2xl border border-gray-100 bg-white shadow-sm p-5 space-y-3">
            <p className="text-[11px] font-bold uppercase tracking-widest text-gray-400">Pago</p>
            ...
            <div className="flex items-center gap-2">
              <span className={cn('inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-semibold', order.payment_status === 'paid' ? 'text-emerald-700 bg-emerald-50 border-emerald-300' : order.payment_status === 'refunded' ? 'text-gray-600 bg-gray-50 border-gray-300' : 'text-yellow-700 bg-yellow-50 border-yellow-300')}>
        ...
        <DialogContent className="sm:max-w-sm p-0 overflow-hidden rounded-2xl duration-200">
          <div className="p-5 space-y-4">
            ...
            <div className="flex items-center gap-2 justify-center py-2">
              ...
              {confirmNewStatus && <StatusBadge status={confirmNewStatus as OrderStatus} />}
            </div>
            <div>
              <label className="text-xs font-medium text-gray-600 mb-1 block">Nota (opcional)</label>
              <Input className="h-9 text-sm rounded-xl border-gray-200" value={confirmNote} onChange={(e) => setConfirmNote(e.target.value)} placeholder="Motivo del cambio…" />
            </div>
            <div className="flex gap-2 pt-1">
              <Button variant="outline" onClick={() => setConfirmOpen(false)} className="flex-1 h-9 rounded-xl text-sm">Cancelar</Button>
              <Button onClick={() => { void changeStatus() }} disabled={confirmLoading} className="flex-1 h-9 rounded-xl text-sm font-semibold">
      ...
      <Dialog open={cancelOpen} onOpenChange={setCancelOpen}>
        <DialogContent className="sm:max-w-sm p-0 overflow-hidden rounded-2xl duration-200">
          <div className="p-5 space-y-4">
            ...
            <p className="text-sm text-gray-600">Esta acción no se puede deshacer. El pedido <span className="font-mono font-bold">{order.short_id}</span> será cancelado.</p>
            <div>
              <label className="text-xs font-medium text-gray-600 mb-1 block">Motivo de cancelación</label>
              <Input className="h-9 text-sm rounded-xl border-gray-200" value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} placeholder="Explica el motivo…" />
            </div>
            <div className="flex gap-2 pt-1">
              <Button variant="outline" onClick={() => setCancelOpen(false)} className="flex-1 h-9 rounded-xl text-sm">Volver</Button>
              <Button onClick={() => { void doCancelOrder() }} disabled={cancelLoading} className="flex-1 h-9 rounded-xl text-sm font-semibold bg-red-600 hover:bg-red-700 text-white">
