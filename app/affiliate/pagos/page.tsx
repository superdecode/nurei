...
import { Download, Receipt, X, FileText, CalendarDays, Tag, Eye } from 'lucide-react'
...
export default function AffiliatePagosPage() {
  ...
  const filteredDetailOrders = useMemo(() => {
    ...
    return (detailPayment.orders ?? []).filter((order) => {
  ...
  }, [detailDateFrom, detailDateTo, detailMaxAmount, detailMinAmount, detailOrderSearch, detailPayment])
                  ...
                  <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50/30 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <button
                        ...
                        className="inline-flex items-center justify-center rounded-full border border-gray-200 p-2 text-gray-500 transition-colors hover:border-primary-cyan hover:text-primary-cyan"
            ...
            <div className="p-5 space-y-4">
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
                <div className="rounded-xl bg-gray-50 px-4 py-3">
                  ...
                  <span className="mt-2 block text-lg font-black text-emerald-700">{formatPrice(detailPayment.amount_cents)}</span>
                </div>
                <div className="rounded-xl bg-gray-50 px-4 py-3">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Tipo</span>
                  <span className="mt-2 block text-xs font-semibold capitalize text-gray-700">{detailPayment.payment_type ?? '—'}</span>
                </div>
                <div className="rounded-xl bg-gray-50 px-4 py-3">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Referencia</span>
                  <span className="mt-2 block text-xs font-mono text-gray-700">{detailPayment.reference_number ?? '—'}</span>
                </div>
